...
router.get("/stock/update/:id", isLoggedIn, async (req, res) => {
  try {
    ...
    if (!itemToUpdate) {
      req.flash(
        "error_msg",
        "Operational Error: That stock profile item could not be found.",
      );
...
router.get("/supplierTable", async (req, res) => {
  try {
    ...
    const supplierCredits = groupedSuppliers.map((group) => {
      const finalStatus = group.statuses.includes("PENDING")
        ? "PENDING"
        : "PAID";
      return {
...
router.get("/supplier/mark-paid/:id", async (req, res) => {
  try {
    await Supplier.updateMany(
      { supplier: req.params.id, status: "PENDING" },
      { $set: { status: "PAID", paymentMethod: "cash" } },
    );
  ...
  async (req, res) => {
    try {
      ...
      const items = toArr(itemName),
        qtys = toArr(quantity),
        costs = toArr(unitCost),
        prices = toArr(unitPrice),
        cats = toArr(category),
        specs = toArr(specification);
